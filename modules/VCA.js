class VCA extends Module {
  constructor() {
    ...
    this.add_control(new Encoder({x:hp2x(0.6), y:46, r:7, vmin:0, vmax:10, val:10, name:'VOL'}));
    this.add_input(new Port({x:hp2x(0.8), y:68, r:6, name:'CV', default_value: 10}));
    this.add_input(new Port({x:hp2x(0.8), y:88, r:6, name:'IN'}));
    this.add_output(new Port({x:hp2x(0.8), y:108, r:6, name:'OUT'}));
    // this.led = new Led({x:hp2x(0.8), y:28, r:3});
    // this.led.set(0);
    // this.attach(this.led);

    // this.button = new Button({x:hp2x(0.8), y:28, r:6});
  ...
  process() {
    ...
    this.out = Math.min(this.i['IN'].get() * ((this.c['VOL'].get()/10 * this.i['CV'].get()/10)), 10);
    this.o['OUT'].set( Math.min(this.i['IN'].get() * ((this.c['VOL'].get()/10 * this.i['CV'].get()/10)), 10) );
