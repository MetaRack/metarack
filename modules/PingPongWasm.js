class PingPongWasm extends Module {
  constructor() {
    super({w:hp2x(4)});
    this.add_control(new Encoder({x:hp2x(0.6), y:6, r:7, vmin:0, vmax:5, val:1, name:'DELAY'}));
    this.add_control(new Encoder({x:hp2x(0.6), y:26, r:7, vmin:0, vmax:1, val:0.5, name:'FB'}));
...
//   }
  ...
  process() {
    ...
    if ((PingPong_done) && (this.done_flag)) {
      // this.dtr_clear();
      this.done_flag = false;
      this.flag = true;
      console.log('No');
      this.ping_pong = new PingPong_Module.PingPong();
      //this.dtr._clear();
    }
    // if (this.flag) {
